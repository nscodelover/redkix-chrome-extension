...
(function() {
     ...
     */
    tagsInput.directive('tagsInput', ["$timeout", "$document", "$window", "tagsInputConfig", "tiUtil",
        function($timeout, $document, $window, tagsInputConfig, tiUtil) {
     ...
     */
    tagsInput.directive('tiTagItem', ["tiUtil",
        function(tiUtil) {
            return {
                restrict: 'E',
                require: '^tagsInput',
     ...
     */
    tagsInput.directive('autoComplete', ["$document", "$timeout", "$sce", "$q", "tagsInputConfig", "tiUtil", "$rxRepoService",
        function($document, $timeout, $sce, $q, tagsInputConfig, tiUtil, RepoService) {
        function SuggestionList(loadFn, options, events, contextObjectUID) {
                var self = {},
            ...
            self.contextObjectUID = contextObjectUID;

                self.reset = function(skipResetPromise) {
                    lastPromise = skipResetPromise ? lastPromise : null;
                ...
                self.load = tiUtil.debounce(function(query, tags) {
                    ...
                    promise.then(function(items) {
                        ...
                        // add async results to sync results and remove duplicates
                        self.items = _.uniq(self.items.concat(items));
                        self.items = self.items.slice(0, options.maxResultsToShow);
                        ...
                        // notify part of promise - deals with sync data received 
                        if (promise !== lastPromise) {
                            return;
                        }
                        ...
                        if (self.items.length > 0) {
                            ...
                            self.reset(true); // true for not reseting the promise as this is just the sync part of the matches
                        }
                    });
            ...
            return {
                ...
                $scope.suggestionList = new SuggestionList($scope.source, $scope.options, $scope.events, $scope.context);
     ...
     */
    tagsInput.directive('tiAutocompleteMatch', ["$sce", "tiUtil",
        function($sce, tiUtil) {
            return {
                restrict: 'E',
                ...
                scope.contextObjectUID = scope.$parent.suggestionList.contextObjectUID;
                scope.liveDisplayName = scope.data.getDisplayName(scope.contextObjectUID);
     ...
     */
    tagsInput.factory('tiUtil', ["$timeout",
        function($timeout) {
            var self = {};
    ...
    tagsInput.run(["$templateCache",
        function($templateCache) {
            $templateCache.put('ngTagsInput/tags-input.html',
                ...
                "<ul class=\"tag-list\"><li class=\"tag-item\" ng-repeat=\"tag in tagList.items track by track(tag)\" ng-class=\"{ selected: tag == tagList.selected }\" " +
                "ng-click=\"eventHandlers.tag.click(tag)\"><ti-tag-item data=\"::tag\"></ti-tag-item></li></ul><input class=\"input\" autocomplete=\"off\" " +
                " ng-model=\"newTag.text\" ng-model-options=\"{getterSetter: true}\" ng-keydown=\"eventHandlers.input.keydown($event)\" ng-focus=\"eventHandlers.input.focus($event)\" " +
                "ng-blur=\"eventHandlers.input.blur($event)\" ng-paste=\"eventHandlers.input.paste($event)\" ng-trim=\"false\" ng-class=\"{'invalid-tag': newTag.invalid}\" ng-disabled=\"disabled\" " +
            ...
            $templateCache.put('ngTagsInput/auto-complete.html',
                ...
                "ng-style=\"{'border-top': $index>0?'1px solid #EAECEE':'0px'}\" ng-click=\"addSuggestionByIndex($index)\" ng-mouseenter=\"suggestionList.select($index)\">" +
