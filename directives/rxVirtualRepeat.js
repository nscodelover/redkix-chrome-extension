/**
 * Override the virtual-repeat directive from angular-material to support elements of different heights.
 * The model now need to support two more methods:
 * 1. getDiffFromConstantHeightTillIndex(index): Should return (fast) the diffenrece between index * constantHeight and the real height. In other words
 *    the height difference between a list with constant item height and the actual list.
 * 2. getTotalHeightDiffFromConstantHeight: Same as previous but for the entire list (not till specific index).
 * @ngdoc module
...
/** @ngInject */
function VirtualRepeatContainerController($$rAF, $parse, $scope, $element, $attrs, $timeout, $rootScope) {
    this.$scope = $scope;
    ...
    $(window).resize(function() {
        if (!self.resizeTimeout) {
            self.resizeTimeout = $timeout(function() {
                self.resizeTimeout = null;
                self.repeater.container.repeater.collectionReset();
            }, 500);
        }
    });
}
VirtualRepeatContainerController.$inject = ['$$rAF', '$parse', '$scope', '$element', '$attrs', '$timeout', '$rootScope'];
...
VirtualRepeatContainerController.prototype.stopScroll = function() {
    this.scrollStopTimeout = null;
    this.$rootScope.$broadcast('rxStartPrefetch');

};

VirtualRepeatContainerController.prototype.isScroll;
VirtualRepeatContainerController.prototype.handleScroll_ = function() {
    ...
    var scrollbarAtEnd = this.scrollSize - this.scrollOffset < this.size;
    if (!scrollbarAtEnd) {
        if (this.scrollStopTimeout) {
            this.$timeout.cancel(this.scrollStopTimeout);
        } else {
            this.$rootScope.$broadcast('rxStopPrefetch');
        }

        this.scrollStopTimeout = this.$timeout(function() {
            self.stopScroll();
        }, 300);
    }
    ...
    var numItems = Math.max(0, this.repeater.items.model.getStartIndexFromOffset(offset) - NUM_EXTRA);

    // Support different heights
    var diffFromItemSize = this.repeater.items.model.getDiffFromConstantHeightTillIndex(numItems);
    this.repeater.items.model.repeaterContainer = this;
    var transform = this.isHorizontal() ? 'translateX(' : 'translateY(';
    transform += (numItems * itemSize + diffFromItemSize) + 'px)';
...
/**
 * The collection was change his size and it's items --> need to measure size again
 */
VirtualRepeatController.prototype.collectionReset = function() {
    var self = this;
    self.container.size = 0;
    self.container.originalSize = 0;
    self.container.$timeout(function() {
...
VirtualRepeatController.prototype.virtualRepeatUpdate_ = function(items, oldItems) {
    ...
    if (this.items && itemsLength < this.items.length && itemsLength > 0 &&
        this.items[0] != items[0] && this.container.getScrollOffset() !== 0) {
        this.items = items;
    ...
    if (lengthChanged) {
        this.container.setScrollSize(itemsLength * this.itemSize + this.items.model.getTotalHeightDiffFromConstantHeight());
    }
...
VirtualRepeatController.prototype.updateIndexes_ = function() {
    ...
    /* (dorn) Reamrk this since the Math.min make us scroll infinitly down, but this is google code so I don't
    want to remove it. The situation where the scrollOffset will be to big to show to much items is does
    not seems possible at this point*/
    /*this.newStartIndex = Math.max(0, Math.min(
        itemsLength - containerLength + 15,
        Math.floor(this.items.model.getStartIndexFromOffset(this.container.getScrollOffset()))));*/
    this.newStartIndex = Math.max(0, Math.floor(this.items.model.getStartIndexFromOffset(this.container.getScrollOffset())));
    var numberOfDiffItems = this.items.model.getNumberOfDiffItemsTillindex(this.newStartIndex + containerLength) - this.items.model.getNumberOfDiffItemsTillindex(this.newStartIndex);
    containerLength += numberOfDiffItems / 2;
    ///  end (dorn)
...
VirtualRepeatModelArrayLike.prototype.$$includeIndexes = function(start, end) {
    ...
    for (var i = start; i < end; i++) {
        if (!this.hasOwnProperty(i)) {
            ...
            if (this[i].diffFromItemSize) {
                this.diffFromItemSize += this[i].diffFromItemSize;
            }
        }
    }
};